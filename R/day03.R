...
f03a <- function(x) {
  items <- sapply(x, f03_get_repeat_items)
  priorities <- sapply(items, f03_item_priorities)
...
f03b <- function(x) {
  elf_groups <- split(x, ceiling(seq_along(x)/3))
  badges <- sapply(elf_groups, f03_get_badge)
  priorities <- sapply(badges, f03_item_priorities)
  return(sum(priorities))
}

#' @examples
#' f03_get_repeat_items("vJrwpWtwJgWrhcsFMMfFFhFp")
f03_get_repeat_items <- function(x) {
  n <- nchar(x)
  r1 <- substr(x, 1, n/2)
  r2 <- substr(x, n/2 + 1, n)

  r1_list <- strsplit(r1,"")[[1]]
  r2_list <- strsplit(r2,"")[[1]]

  return(intersect(r1_list,r2_list))
}

#' @examples
#' f03_item_priorities("a")
#' f03_item_priorities("A")
#' f03_item_priorities("Z")
f03_item_priorities <- function(x) {
  if(x %in% letters){
    return(match(x, letters))
  } else if(x %in% LETTERS){
    return(match(x, LETTERS) + 26)
  } else{
    return(NA)
  }
}
...
#' @examples
#' f03_get_badge(example_data_03()[1:3])
#' f03_get_badge(example_data_03()[4:6])
f03_get_badge <- function(bag_list){
  item_list <- strsplit(bag_list,"")
  Reduce(intersect, item_list)
}
