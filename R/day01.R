...
#' @export
#' @examples
#' test <- f01_get_input(example_data_01())
#' f01a(example_data_01())
#' f01b(example_data_01())
f01a <- function(x) {
  input <- f01_get_input(x)
  cal_sums <- sapply(input,sum)
  return(max(cal_sums))
}
...
f01b <- function(x) {
  input <- f01_get_input(x)
  cal_sums <- sapply(input,sum)
  cal_sum_sort <- sort(cal_sums, decreasing = TRUE)
  return(sum(cal_sum_sort[1:3]))
  }

#' @examples
#' f01_get_input(example_data_01())
f01_get_input <- function(x) {
  cal_list <- unlist(strsplit(readr::read_file(x),"\n\n"))
  cal_list2 <- type.convert(strsplit(cal_list, "\n"), as.is = TRUE)
  return(cal_list2)
}
...
example_data_01 <- function(example = 1) {
  here::here("inst","testdata",paste0("testdata01.",example,".txt"))
}
