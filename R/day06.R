...
#' @export
#' @examples
#' f06a(example_data_06()[1])
#' f06b(example_data_06()[1])
f06a <- function(x) {
  f06_scan_for_signal(x, n_unique = 4)
}
...
#' @export
f06b <- function(x) {
  f06_scan_for_signal(x, n_unique = 14)
}

#' @examples
#' f06_scan_for_signal(example_data_06()[[1]])
f06_scan_for_signal <- function(x, n_unique){
  d <- n_unique -1
  x <- unlist(strsplit(x,""))
  ...
  for(i in 1:(length(x)-d)){
    x_sub <- x[i:(i+d)]
    # message(x_sub)
    if(!any(duplicated(x_sub))) return(i+d)
  }
